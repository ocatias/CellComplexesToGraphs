...
def main(args):
    ...
    if args.model.startswith('gin'):  # load graph dataset
        ...
        test_loader = None
        if test_split is not None and args.tune_params == 0:
    ...
    # Store results
    results_path = f"../../results/CWN/{args.dataset}"
    if not os.path.isdir(results_path):
        os.mkdir(results_path)

    if args.tune_params == 0:
        # Store params and test results
        with open(os.path.join(results_path, f"Fold_{args.fold}_results.csv"), "a") as file:
            file.write(str(test_curve[best_val_epoch]) + "\n")

    else:
        # Store params and validation results
        directory = os.path.join(results_path, f"Fold_{args.fold}")
        ...
        data = {"args": args.__dict__,  "vaL_result": valid_curve[best_val_epoch]}
        print(data)
